...
def figure(xlabel, ylabel, plot_title, axis_color):
    # formatting
    axis_font_size = 10
    line_thk = 1.5
    ...
    # set labels
    plt.xlabel(xlabel, fontsize=axis_font_size * 1.125, fontweight="bold", color=axis_color)
    plt.ylabel(ylabel, fontsize=axis_font_size * 1.125, fontweight="bold", color=axis_color)
    plt.title(plot_title, fontsize=axis_font_size * 1.5, fontweight="bold", color="k")
...
if __name__ == "__main__":
    ...
    # locate data
    path = os.path.join("C:\\", "20180912", "AR")
    data_location = diodes.find_diode_data(path)[2]
    data = diodes.load_diode_data(data_location, apply_lowpass=True)
    ...
    [plot, axis] = figure(x_lab, y_lab, title, ax_color)

    axis.plot(time, data)
    axis.legend(["Diode 0", "Diode 1"], loc=4, edgecolor="none")
    ...
    arrow_kwargs = {
        ...
        "head_width": 0.01,
        "head_length": gap_width / 10.0,
        "overhang": 0.5,
    }
    axis.arrow(line_times[0] + gap_width / 2, 0.25, gap_width / 2 * 0.9, 0, **arrow_kwargs)
    axis.arrow(line_times[0] + gap_width / 2, 0.25, -gap_width / 2 * 0.9, 0, **arrow_kwargs)
    axis.text(
        line_times[0] + gap_width / 2,
        0.25,
        r"$\Delta t$",
    ...
    axis.plot(norm_diffs[(norm_diffs.index > min_time) & (norm_diffs.index <= max_time)])
    ...
    axis.plot(time, data)
    axis.legend(["Diode 0", "Diode 1"], loc="upper left", bbox_to_anchor=(0, 1.1), facecolor="none", edgecolor="none")
