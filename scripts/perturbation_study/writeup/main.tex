...
\title{What is a sensitivity coefficient anyway?}
\author{Mick Carter}
\date{}
...
In both cases we begin by calculating a sensitivity coefficient $C$, which is generally computed as

\begin{equation}
    \label{eq:sc_base}
    \overrightarrow{C} = \frac{\lambda_{u} - \overrightarrow{\lambda}_{p}}{\overrightarrow{\Delta k}}
\end{equation}

\noindent where the subscripts $u$ and $p$ correspond to unperturbed and perturbed simulations, and $k$ corresponds to some per-reaction value which we use as a basis for calculating sensitivity.
The derivation of $\overrightarrow{\Delta k}$ is discussed in sections~\ref{sec:mul_approach} and~\ref{sec:rcc_approach}.
The coefficient is then normalized by the unperturbed cell size to nondimensionalize it, allowing it to be compared across conditions.
...
\section{Reaction Rate Multiplication Factor}
\label{sec:mul_approach}

This section discusses the derivation and application of a cell size sensitivity coefficient calculated on the basis of the multiplication factor used as an input to perturb to each reaction pathway.

\subsection{Derivation}\label{subsec:sec:mul_derivation}
...
\subsection{Application}\label{subsec:mul_application}

Figures~\ref{fig:sc_mul} and~\ref{fig:nsc_mul} show non-normalized and normalized sensitivity coefficients using the multiplication factor as a basis.
Cell sizes were simulated for a stoichiometric mixture of \ce{CH4}-\ce{N2O} at 300 K and 1 atm using the \texttt{gri30\_highT.yaml} mechanism~\cite{gri3,caltech_mech_list} using Cantera~\cite{cantera} and SDToolbox~\cite{sdtoolbox}.

\begin{figure}[H]
    \label{fig:sc_mul}
    \centering
    \includegraphics[width=3in]{images/coefficients/mul/CO2/non_normalized}
    \includegraphics[width=3in]{images/coefficients/mul/N2/non_normalized}
    \caption{Non-normalized multiplication-factor-based sensitivity coefficients (eq. \ref{eq:sc_mul}).}
\end{figure}
...
\begin{figure}[H]
    ...
    \centering
    \includegraphics[width=3in]{images/coefficients/mul/CO2/non_normalized}
    \includegraphics[width=3in]{images/coefficients/mul/N2/normalized}
    \caption{Normalized multiplication-factor-based sensitivity coefficients (eq. \ref{eq:nsc_base},~\ref{eq:sc_mul}).}
\end{figure}

\section{Relative Chemical Contribution (RCC)}
\label{sec:rcc_approach}

This section discusses the derivation and application of a cell size sensitivity coefficient calculated on the basis of the relative chemical contribution of each reaction pathway as a result of having its reaction rate scaled, after calculation, by a multiplication factor.

\subsection{Derivation}
\label{subsec:rcc_derivation}
...
\label{subsec:rcc_application}

Figures~\ref{fig:sc_rcc} and~\ref{fig:nsc_rcc} show non-normalized and normalized sensitivity coefficients using the relative chemical contribution as a basis.
Cell sizes were simulated for a stoichiometric mixture of \ce{CH4}-\ce{N2O} at 300 K and 1 atm using the \texttt{gri30\_highT.yaml} mechanism~\cite{gri3,caltech_mech_list} using Cantera~\cite{cantera} and SDToolbox~\cite{sdtoolbox}.

\begin{figure}[H]
    \label{fig:sc_rcc}
    \centering
    \includegraphics[width=3in]{images/coefficients/RCC/CO2/non_normalized}
    \includegraphics[width=3in]{images/coefficients/RCC/N2/non_normalized}
    \caption{Non-normalized RCC sensitivity coefficients (eq. \ref{eq:sc_rcc}).}
\end{figure}
...
\begin{figure}[H]
    ...
    \centering
    \includegraphics[width=3in]{images/coefficients/RCC/CO2/non_normalized}
    \includegraphics[width=3in]{images/coefficients/RCC/N2/normalized}
    \caption{Normalized RCC sensitivity coefficients (eq. \ref{eq:nsc_base},~\ref{eq:sc_rcc}).}
\end{figure}
