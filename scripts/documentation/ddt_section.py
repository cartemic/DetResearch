...
mech = "gri30.yaml"  # mechanism for Cantera calculations
gas = ct.Solution(mech)
equivs = np.linspace(0.4, 1.0, 20)  # desired equivalence ratios
fuels = ["CH4", "C3H8"]  # fuels to test with
fuel_names = {"CH4": "Methane", "C3H8": "Propane"}
oxidizer = "N2O"

# find spiral diameter resulting in desired blockage ratio
spiral_diam = bd.tube.DDT.calculate_spiral_diameter(tube_diam, desired_br)
print("Spiral diameter: {0:.2f} ({1:.2f})".format(spiral_diam.to("cm"), spiral_diam.to("in")))
...
print("Calculated diameter: {0:.2f} ({1:.2f})".format(washer_diam.to("cm"), washer_diam.to("in")))

# round washer diameter to the nearest 1/4 inch
washer_diam = np.round(washer_diam.to("in") * 4) / 4.0
print("Washer diameter: {0:.2f} ({1:.2f})".format(washer_diam.to("cm"), washer_diam.to("in")))

# calculate new blockage ratio
actual_br = num_struts * (washer_diam.to("m") / tube_diam.to("m")).magnitude ** 2
print("Actual blockage ratio: {:.2f}%".format(actual_br * 100))

# calculate run-up distance as a function of equivalence for each mixture
axis_color = "#333333"
fig, axes = figure("Equivalence Ratio", "Run-up Distance (m)", "DDT Section Minimum Length", axis_color)
for fuel in fuels:
    print(fuel)
    runup_lengths = []
    ...
    for phi in equivs:
        ...
        runup = bd.tube.DDT.calculate_run_up(actual_br, tube_diam, init_temp, init_press, species, mech, ureg)
        runup_lengths.append(runup)
        print("    {0}: {1:.2f} ({2:.2f})".format(phi, runup, runup.to("ft")))

    axes.plot(equivs, [length.to("m").magnitude for length in runup_lengths], label=fuel_names[fuel])
