...
# Experimental data
dir_data = os.path.join("/d/", "Data", "Processed", "Data")
...
dates_to_process = sorted(set(df_day["date"].values))
df_day = df_day[df_day["date"].isin(dates_to_process)]
# df_day["success"] = df_day["schlieren"].notna()


# cut down data
low_equivalence_threshold = 0.85
df_day = df_day[
    df_day["schlieren"].notna()  # only shots with schlieren data
    & (df_day["fuel"] == "C3H8")  # propane
    & (df_day["oxidizer"] == "air")  # air
    & (df_day["phi_nom"] > low_equivalence_threshold)  # Throw out low ER data
    & ~(  # Get rid of high dilution at low ER
        (df_day["diluent"] != "None") & (np.isclose(df_day["phi_nom"], 0.9)) & (np.isclose(df_day["dil_mf_nom"], 0.02))
    )
]

df_schlieren_uncert = funcs.uncertainty.df_merge_uncert(
    df_schlieren.dropna(), ["loc_px", "delta_px", "spatial_centerline"]
)

# turn locations into cell sizes
df_schlieren_uncert["measured"] = df_schlieren_uncert["delta_px"] * df_schlieren_uncert["spatial_centerline"] * 2
...
# Probably not worth fixing overwrite at this point
# ruff: noqa: PLW2901
for (_dil, _phi, _mf), df_day_red in df_day.groupby(["diluent", "phi_nom", "dil_mf_nom"]):
    if len(df_day_red):
        # collect population standard deviation of measurements and turn into CI
        t_stat = t.ppf(0.975, len(df_day_red) - 1)
        series_pop_std = pd.Series((np.nanstd(df_day_red[c].values) * t_stat for c in red_columns), index=red_columns)
        ...
        series_mean = pd.Series(
            [_dil, *(np.nanmean(df_day_red[c].values) for c in red_columns), np.nan], index=all_out_columns
        )
        ...
        series_mean["cell_size"] = estimator(df_schlieren_red["measured"].values)
        #         print(np.median(df_schlieren_red["measured"].values))
        legend_label = "ϕ={:1.1f}, dil={:s}, CO2 equivalent={:3.2f}".format(_phi, _dil, _mf)

        # EXPERIMENTING WITH DISTRIBUTIONS
        plt.figure()
        sns.distplot(unp.nominal_values(df_schlieren_red["measured"]))
        plt.axvline(np.median(unp.nominal_values(df_schlieren_red["measured"])), ls=":")
        plt.axvline(np.mean(unp.nominal_values(df_schlieren_red["measured"])), ls="--")

        plt.figure()
        medians = (
            df_schlieren_red["measured"]
            .apply(lambda x: x.nominal_value)
            .rolling(len(df_schlieren_red), min_periods=0)
            .median()
            .reset_index(drop=True)
        )
        ((medians.iloc[:-1] - medians.iloc[-1]).abs() / medians.iloc[-1] * 100).plot(
            style="-", alpha=0.5, mew=0, label=legend_label, figsize=(18, 4)
        )

        df_out = df_out.append(series_mean, ignore_index=True)

        percent_cutoff = 2
        plt.axhline(percent_cutoff, ls="--", c="k", alpha=0.25, zorder=-1, label="{:0.2f} %".format(percent_cutoff))
        plt.xlabel("Measurement #")
        plt.ylabel("Absolute Estimator\nError (%)")
        plt.title(
            f"Convergence of measured cell sizes\n" rf"$\phi$={_phi}, diluent={_dil}, CO2e mf={_mf}", weight="bold"
        )
...
df_plot_base = funcs.uncertainty.df_split_uncert(df_out, ["p_0", "t_0", "phi", "dil_mf", "wave_speed", "cell_size"])
...
def convert_to_co2(row):
    if row["diluent"] == "None":
        ...
        if "u_dil_mf" in row:
            dil_mf_co2_perturbed = funcs.specific_heat_matching.match_adiabatic_temp(
                "gri30.yaml",
                "C3H8",
                "O2:1 N2:3.76",
                row["phi"],
...
def split_diluent_dataframes(df):
    _df_co2 = df[(df["diluent"] == "CO2") | (df["diluent"] == "None")].copy()
    _df_n2 = df[(df["diluent"] == "N2") | (df["diluent"] == "None")].copy()
    _df_co2["diluent"] = "CO2"
    _df_n2["diluent"] = "N2"
...
fig.set_axis_labels("Mole Fraction (CO2e)", "Cell Size")
plt.suptitle(f"Schlieren Measurements, C3H8-air ({datetime.datetime.today().date()})", weight="bold")
plt.tight_layout()
...
df_plot_diluted_sim = df_plot_diluted_sim.melt(
    ["mechanism", "diluent", "dil_mf", "cj_time", "cj_speed", "cell_time", "phi", "dil_mf_CO2e"],
    var_name="Method",
