                    ...
                    for k in range(-window, window + 1):
                        ...
                        for l in range(-window, window + 1):
                            if (i + k >= 0) & (i + k < img_h) & (j + l >= 0) & (j + l < img_w):
                                local_sum += psd[i + k - 1, j + l - 1] / win_size
...
def get_radial_intensity(
    ...
    return (np.arange(rot.shape[1]) - xc, rot[yc, :])


def peaks_to_measurements(peak_x, delta_px, delta_mm, theta, img_square_size):
    px_perpendicular = img_square_size / np.where(np.isclose(peak_x, 0), np.nan, peak_x)
...
def get_reject_mask(thetas, radii, mask_angles, angle_pm, safe_rad):
    ...
    for a in mask_angles:
        if a < angle_pm:
            mask_out[
                ((thetas < np.mod(a + angle_pm, 360)) | (thetas > np.mod(a - angle_pm, 360))) & (radii > safe_rad)
            ] = False
        else:
            mask_out[
                ((thetas < np.mod(a + angle_pm, 360)) & (thetas > np.mod(a - angle_pm, 360))) & (radii > safe_rad)
...
def get_pass_mask(thetas, radii, mask_angles, angle_pm, safe_rad):
    ...
    for a in mask_angles:
        if a < angle_pm:
            mask_out[((thetas < np.mod(a + angle_pm, 360)) | (thetas > np.mod(a - angle_pm, 360)))] = True
        else:
            mask_out[((thetas < np.mod(a + angle_pm, 360)) & (thetas > np.mod(a - angle_pm, 360)))] = True
